 ...
 */
public class CargoAutoCompleteTextView extends FrameLayout {

    private AutoCompleteTextView autoCompleteTextView;
    private TextInputLayout textInputLayout;
    private CargoArrayAdapter adapter;
    private int filterType = AutocompleteFilter.TYPE_FILTER_CITIES;

    public CargoAutoCompleteTextView(@NonNull Context context) {
        super(context);
        init();
    }

    public CargoAutoCompleteTextView(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public CargoAutoCompleteTextView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    void init() {
        View view = inflate(getContext(), R.layout.cargo_auto_complete_view, this);

        autoCompleteTextView = view.findViewById(R.id.auto_complete_text_view);
        textInputLayout = view.findViewById(R.id.text_input_layout_2);

        autoCompleteTextView.setThreshold(2);
        adapter = new CargoArrayAdapter(getContext());
